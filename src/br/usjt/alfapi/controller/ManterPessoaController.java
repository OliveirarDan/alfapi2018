...
public class ManterPessoaController {
	...
	public String criarPessoaFoto64(@Valid Pessoa pessoa, @RequestParam("file") String string64, BindingResult erros,
			Model model) {
		try {
			if (!erros.hasErrors()) {
				// insere e pega o endereço cadastrado (ID_endereco é necessário)
	...
	public String pegaFoto64(Pessoa pessoa, @RequestParam("file") String string64, BindingResult erros, Model model) {
		try {
			if (!erros.hasErrors()) {
				// Prepara a string do base64
				String base64Image = string64.split(",")[1];
	...
	/*
	 * Método inserirPessoa2 que deve receber um Json com o objeto pessoa para
	 * cadastro Headers: utilizado para que o método possa reconhecer que irá
	 * receber um Json. Observe que no GET não foi necessário (talvez em outros
	 * casos seja.). ResponseBody: qual será o retorno do método. RequestBody: o que
	 * p método irá receber no corpo do Json. **Observe que precisa ser definido o
	 * que irá no corpo do response e do request.
	 * 
	 * PROBLEMA: está informando um erro do tipo Hibernate no entity Endereço. Acho
	 * que é algo sobre o relacionamento entre Endereco e Pessoa. Precisa validar se
	 * a classe entity.Endereco e Pessoa estão anotadas corretamente.
	 */

	@Transactional
	// @RequestMapping(method = RequestMethod.POST, value = "rest/pessoa", headers =
	// "Accept=application/json")
	// public @ResponseBody Pessoa inserirPessoa(@RequestBody Pessoa pessoa, Model
	...
	/*
	 * Método para buscar pessoa por Id. RequestMapping é necessário para mapear o
	 * tipo de requisicao ResponseBody: determina qual será o corpo da resposta, ou
	 * seja, o que irá no retorno do método como Json PathVariable: usado para
	 * identificar o id como atributo e não um recurso do método * Return: um Json
	 * com o objeto pessoa que foi identificado pelo id.
	 */
